...
LPR 是一個以「教案上傳 -> AI 分析與評論 -> 人工補充評分」為核心流程的教學輔助系統。
...
5. 重新上傳與刪除教案紀錄採用導覽列全域入口，避免跨頁切換與重複操作。
...
2. **導覽操作整合與精簡** (2026/04)：
   - 移除獨立查看教案頁（lesson-view），改為四頁流程。
   - 將「重新上傳」「刪除教案與評論紀錄」整合到主要頁面的側邊導覽。
   - 對應文件：lesson-review-ui/index.html、lesson-review-ui/lesson-review.html、lesson-review-ui/lesson-score.html、lesson-review-ui/lesson-shared-actions.js、lesson-review-ui/styles.css
   - 效果：降低跨頁跳轉與功能分散，操作入口一致。
...
18. 側欄展開狀態為左對齊、收合狀態為 icon 置中；登入/登出按鈕外框貼合按鈕形狀。
